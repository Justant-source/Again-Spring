...
/**
 * 세션 카테고리(대분류 > 중분류 > 소분류 + 직접 입력)를 LLM 프롬프트에 주입.
 * 권위본: shared/docs/policies/categories.md §"LLM 프롬프트 주입 형식"
 *
...
public class CategoryContextFragment {
    ...
    public String render(Session session) {
        ...
        CategoryCatalog.MiddleCategory middle = c.middleId != null
                ? catalog.getMiddle(c.majorId, c.middleId) : null;
        CategoryCatalog.MinorCategory minor = (c.middleId != null && c.minorId != null)
                ? catalog.getMinor(c.majorId, c.middleId, c.minorId) : null;

        StringBuilder sb = new StringBuilder();
        sb.append("<conflict_category note=\"사용자가 세션 시작 시 명시적으로 선택한 갈등 분류. ")
          .append("이를 인지하여 카테고리에 적합한 후속 질문을 생성하세요. ")
          .append("단, '당신이 부부 카테고리를 선택하셨네요' 같이 라벨을 직접 인용하지는 않습니다.\">\n");
        sb.append("- 관계 유형: ").append(major.getLabel()).append("\n");

        if (middle != null) {
            sb.append("- 갈등 카테고리: ").append(middle.getLabel());
            if (minor != null && !"custom".equals(minor.getId())) {
                sb.append(" > ").append(minor.getLabel());
            }
            sb.append("\n");
        }
