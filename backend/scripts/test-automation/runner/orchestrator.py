...
class Orchestrator:
    def __init__(self, dev_url: str = DEV_URL, max_concurrent: int = MAX_CONCURRENT):
        ...
        self.results = []
        # 같은 A-user가 세션 한도를 초과하지 않도록 사용자별 직렬 실행 보장
        self._user_semaphores: dict[str, asyncio.Semaphore] = {}

    def _user_sem(self, email: str) -> asyncio.Semaphore:
        if email not in self._user_semaphores:
            self._user_semaphores[email] = asyncio.Semaphore(1)
        return self._user_semaphores[email]
    ...
    async def _run_one_wrapped(self, run: dict, http: aiohttp.ClientSession,
        ...
        async with self.semaphore:
            async with self._user_sem(run["email"]):
    ...
    async def _run_duo_pair(self, run: dict, http: aiohttp.ClientSession,
        ...
        """
        async with self.semaphore:
            async with self._user_sem(run["duo_email_a"]):
                scenario = run["scenario"]
                ...
                try:
                    ...
                    # Duo 세션 cleanup: TERMINATED 강제 종료
                    try:
                        await bot_a.client.admin_terminate_session(session_id)
                    except Exception:
                        pass
    ...
    def _categorize_scenario(self, sc_id: str) -> str:
        if sc_id.startswith("SC-CANCEL"):
            return "mechanism"
        elif sc_id.startswith("SC-FLOW"):
            return "flow"
        elif sc_id.startswith("SC-VALID"):
            return "validation"
        elif sc_id.startswith("SC-CTX"):
            return "context"
        # Legacy numeric IDs
        try:
            num = int(sc_id[2:])
    ...
    def _build_summary(self, results: list) -> dict:
        ...
        for r in results:
            ...
            cat = self._categorize_scenario(sc_id)
