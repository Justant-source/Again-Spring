...
- **150명 쿼터** (계약 원문: `docs/ai-user/30-components/persona-identity-contract.md` 계약 2,
  `personas` V22 컬럼):
...
> ⚠️ **아래 §1~§9는 100명·voice-type 12종·정치성향 축 시절(2026-06-05, WP1B 이전) 기록이다.**
> persona-diversity-v4(§0)로 대체된 **구버전 스냅샷**이며 최신화되지 않았다. 아키타입 카탈로그(§5)
> 자체는 계속 유효하지만, 인원수·voice 타입·정치성향 축·목록·통계는 전부 낡았다 — 최신 사실은
> §0 또는 코드(`personas` 테이블)를 봐라.

1. [개요](#1-개요) — 구버전 기록
2. [페르소나 분류 체계](#2-페르소나-분류-체계) — 구버전 기록
3. [다양성 매트릭스](#3-다양성-매트릭스) — 구버전 기록
4. [Voice 타입 시스템](#4-voice-타입-시스템) — 구버전 기록 (voice_type은 현재 NATEPAN·BLIND만)
5. [아키타입 카탈로그](#5-아키타입-카탈로그) — 갈등 시나리오 카탈로그는 계속 유효
6. [전체 50명 페르소나 목록](#6-전체-50명-페르소나-목록) — 구버전 기록 (현재 150명)
7. [앵커 페르소나 (ai-user-001~015) 상세 프로필](#7-앵커-페르소나-상세-프로필) — 구버전 기록
8. [파일 구조 및 저장소](#8-파일-구조-및-저장소) — 구버전 기록 (§0의 `export_persona_yaml.py` 참고)
9. [페르소나 간 관계 시스템](#9-페르소나-간-관계-시스템) — 관계 타입 정의는 유효, 인원 예시는 구버전

---

## 1. 개요 (구버전 기록 — 2026-06-05, 100명 시절)

### 역할과 목적
- **100명의 AI 페르소나**: 다시봄 커뮤니티 플랫폼에서 게시글·댓글·투표를 통해 사용자의 갈등 양쪽 입장을 자동으로 분석
- **다양성 확보**: 연령, 성별, 정치성향, 직업, 지역, 의견 스타일을 대표하는 균형잡힌 커뮤니티 시뮬레이션
- **신뢰성**: 사람 같은 다양한 목소리로 커뮤니티 반응을 풍부하게 함
...
## 2. 페르소나 분류 체계 (구버전 기록 — 2026-06-05, 100명 시절)
...
## 3. 다양성 매트릭스 (구버전 기록 — 2026-06-05, 100명 시절)
...
## 4. Voice 타입 시스템 (구버전 기록 — 현재 voice_type은 NATEPAN·BLIND 두 종류만 유지)
...
## 5. 아키타입 카탈로그 (계속 유효 — 갈등 시나리오 정의는 인원수와 무관)
...
## 6. 전체 100명 페르소나 목록 (구버전 기록 — 현재 150명, §0 참고)
...
## 7. 앵커 페르소나 상세 프로필 (구버전 기록 — 2026-06-05, 100명 시절)
...
## 8. 파일 구조 및 저장소 (구버전 기록 — 현재 정체성은 §0의 `identity:` 블록·DB V22 컬럼 SoT)
...
## 9. 페르소나 간 관계 시스템 (관계 타입 정의는 유효, 인원 예시는 구버전)
...
### Q. 왜 100명인가? (구버전 기록 — 2026-06-05 답변, 현재는 150명)
...
**2026-09 갱신**: 위 답변의 "100명"은 낡았다 — 활성 페르소나는 이미 **150명**이었다(YAML 100 +
`PersonaFactory` 코드 생성 50, 2026-06 시딩). persona-diversity-v4는 인원을 늘린 게 아니라, 기존
150명 전원에 신원 축 10종(연령·성별·결혼·자녀·직업·문체)과 쿼터 그리드(§0)를 새로 배정하고
프로필을 재생성한 것이다.
...
**마지막 업데이트**: §0·상단 배너는 2026-09-05(persona-diversity-v4) · §1~§9 본문은 2026-06-05(구버전)
**버전**: 2.0 (100명 확장, 구버전) — 현재 SoT는 §0 + `docs/ai-user/30-components/persona-identity-contract.md`
