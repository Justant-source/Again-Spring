...
| `{dynamicExamples}` | 유사 커뮤니티 예시 (RAG, 정규화 처리) | Learning 서비스 (RAG 검색) |
...
## 7.3 동적 예시 정규화 (RAG 출력 처리)

PromptAssembler의 `dynamicExamplesBlock()` 메서드:

```java
// RAG 크롤링 예시에서 온점·쌍따옴표 정규화
...
**목적**:
- RAG 학습 데이터는 실제 커뮤니티 글이므로 온점과 쌍따옴표가 포함될 수 있음
- 이를 그대로 주입하면 모델이 금지된 문체를 모방할 가능성 ↑
- 따라서 삽입 전에 온점과 쌍따옴표를 제거하고, 명확한 구분선(───)과 라벨로 격리
...
**효과**:
- 구조·길이만 참고하도록 명시
- 문체 규칙은 위의 System 프롬프트만 따르도록 강제 격리

---

## 8. 핵심 3가지 원칙 (2026-06-05 개정)
...
→ 온점 대신 줄바꿈, ㅠ, ㅋ, ... 으로 끊기
→ 문장 끝 온점 정규식으로 자동 제거 검사 (OutputSanitizer 4단계)
→ 자기비평(Self-Critique)에서 -2점 감점
예외: 물음표(?), 느낌표(!), 말줄임표(...) 사용 가능
```

**강화 사항 (2026-06-05)**:
- 글·댓글·대댓글 user 프롬프트 끝에 "⚠️ 문장 끝 온점(.) 금지·쌍따옴표 금지 — 한국 커뮤니티 문체만 따를 것" 명시
- 존댓말·반말 모두에 동일 적용 (문체 차이 없음)

### 9.2 쌍따옴표("") 사용 금지 (간접화법 인용)

```
금지: 남자친구가 "전여친이 더 예뻤다"고 했어 / "바빠서 못 봤다"며 연락이 없음
허용: 남자친구가 전여친이 더 예뻤다고 했어 / 바빠서 못 봤다며 연락이 없음
허용: 남자친구가 그냥 지나가는 말이라고 함 / 걔가 뭐라고 했냐면
대체: ~라고 하더라고요 (존댓말) / ~라고 함 (반말)
```

**강화 사항 (2026-06-05)**:
- 프롬프트에서 쌍따옴표 예시 자체도 제거 (슬래시 구분으로 변경)
- "진짜 공감해요" → "진짜 공감해요" (따옴표 없음)
- 자기비평(Self-Critique)에서 -2점 감점
...
**마지막 업데이트**: 2026-06-05 (PromptAssembler 온점·따옴표 강화)
