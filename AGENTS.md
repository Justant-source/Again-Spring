...
- Codex는 `.claude/settings.local.json`의 allow/deny 의도를 운영 규칙으로 따른다: 비파괴적 작업은 가능한 한 사용자 확인 없이 계속 진행하고, 파괴적 git 작업은 동일하게 금지한다. 단, Codex 플랫폼 승인/샌드박스 자체를 이 파일로 비활성화할 수는 없다.
