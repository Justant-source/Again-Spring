...
**ASM**: Python 3.12 + FastAPI, 포트 8200. AS 호스트에서 `ssh justant@100.115.252.61` **암호 없이** 접속. **활성 = X / `x_thread` + Instagram(`instagram_feed`·`instagram_reels`) + YouTube Shorts / `youtube_shorts`** (게시 후 24h 자동, 2026-09-04 실측 재확인 — YouTube는 계속 자동 생성·게시 중이었고 `youtube-shorts-strategy.md`가 권위본대로 활성 상태였음). 네이버·Threads 보류. Again-Spring 쪽은 thin client만.
