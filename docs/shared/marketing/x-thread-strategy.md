...
그 외 트윗(reply)은 텍스트 없이 이미지 단독. 이미지는 다시봄 실제 화면의 무보정 캡처.
해시태그(브랜드 2만): 메인 또는 체인 말미에 `#다시봄` `#againspring` — 카테고리 태그 없음 ([`platforms.md`](platforms.md)).
체인 말미 링크에는 UTM 부착 → 사연 상세 랜딩 (Phase 1 계측).
...
|---|---|---|---|---|
| 메인~ | 작성자 본문 | 작성자 part1..N | 작성자 parts → 상대 parts | `/read?side=g` then `r` |
| … | 광장 댓글(≤4) | … | 광장 댓글(≤4) | `/community/{id}` |
| 마지막 | 공감 비율 + 링크 | 공감 비율 + 링크 | 공감 비율 + 링크 | `/community/{id}` |

양면 최악 예: 작성자3 + 상대3 + 댓글 + 비율 (훅은 IG만). X는 비율이 마지막 답글.
