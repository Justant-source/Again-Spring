...
7. 최종 플랜이 비었거나 최소 장수에 못 미치면 **렌더를 시작하지 않고 FAILED**로 끝낸다. Reels는 4장, Shorts는 5장이 최소다. 크림+텍스트 전용 폴백과 메타포 폴백은 모두 금지한다.
...
AS는 채널별 초기 생성 결과가 비었거나 최소 장수에 못 미친 경우에만, 부족한 플랜을 명시한 **LLM 보정 호출 1회**를 허용한다. 보정 뒤에도 가드를 통과하지 못하면 폴백 없이 실패하며 각 시도의 안전한 요약 진단만 남긴다.
