...
`personaLearningEnabled` 기본 **true**, `personaLearnAt` 기본 `04:30` KST. 분 단위 스케줄이 그 시각에 한 번 `@againspring_net` 타임라인(FxTwitter)을 읽어 **남을 향한 수동 댓글·인용**만 코퍼스에 넣고, prod에서만 Haiku로 프로필을 증류한다. 자동 스레드(자기 답글·URL만·`#다시봄`/`#againspring` 훅)는 버린다. 코퍼스는 `x_persona_example`. Telegram `/drill`은 같은 후보 API로 상황을 보여 주고 운영자 한 줄을 `DRILL`(가중 2)로 저장한다. GET `x-ops`의 `personaDrillToday`는 오늘 KST 드릴 건수.
