...
| IG | 솔로 짧은 | 솔로 긴 | 양면(둘 다 짧음) | 양면(둘 다 김) |
|---|---|---|---|---|
| 1 | 훅 카드 (4:5) | 훅 카드 (4:5) | 훅 카드 | 훅 카드 |
| 2 | `storyPart1` | `storyPart1` | `storyPart1` | `storyPart1` |
| 3 | `comments` | `storyPart2` | `partnerPart1` | `storyPart2` |
| 4 | 비율 디자인 카드 | `comments` | `comments` | `partnerPart1` |
| 5 | — | 비율 디자인 카드 | 비율 디자인 카드 | `partnerPart2` |
| 6 | — | — | — | `comments` |
| 7 | — | — | — | 비율 디자인 카드 |

실측 장수 = **솔로 4~5**, **양면 5~7**(작성자·상대 본문 길이 조합). IG 캐러셀 상한(10) 안.
상대방 본문(`partnerPart*`)은 BE brief `has_partner_story=true`일 때만 `/read?side=r`에서 캡처해
작성자 본문 카드 **바로 뒤**에 이어 붙인다.

중간 장(X 캡처)은 원본을 `#FAF7F4` 4:5 캔버스에 두고 **여백(~40px) + `object-fit: contain`** 으로 올린다.
**사연 전반/후반의 분홍·세이지(또는 캡처) 레이아웃은 절대 크롭하지 않는다** — 본문 UI 전체가 보여야 한다.
