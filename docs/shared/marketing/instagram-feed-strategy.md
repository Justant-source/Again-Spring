...
|---|---|---|
| 1 | 훅 카드 (4:5) | 훅 카드 (4:5) |
| 2 | `storyPart1` (원본 비율) | `storyPart1` (원본 비율) |
| 3 | `comments` (원본 비율) | `storyPart2` (원본 비율) |
| 4 | 비율 디자인 카드 (4:5) | `comments` (원본 비율) |
| 5 | — | 비율 디자인 카드 (4:5) |
...
- 크기: `1080×1350` (4:5)
- 내용: 작성자/상대방 막대 + `A% : B%` 숫자만
- 진영색: 작성자 `#C9785A` / 상대방 `#5F8F76`
- 판결·승패·설명 문구·로고·CTA 오버레이 금지 (CTA는 캡션 URL이 담당)
