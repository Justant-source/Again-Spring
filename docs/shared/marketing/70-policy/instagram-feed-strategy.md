...
- 훅·비율 HTML 카드: renderer `/render`에 `deviceScaleFactor: 3` 후 CSS 뷰포트(1080×1350)로 **다운스케일** (1x 스크린샷을 키우지 않음 — X 캡처와 동일).
- 사연 분홍 박스는 캔버스 **중앙 1:1 (y=135..1215)** 안에 `contain`. **업스케일 금지** (`scale ≤ 1`). 1080보다 큰 캡처만 축소. 카드 crop은 JPEG 재인코딩 없이 PNG.
