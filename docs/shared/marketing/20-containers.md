...
`GET`/`PUT /api/admin/marketing/x-ops`(`marketing.x.*`)에 있다. **발행 스위치 기본 꺼짐.** 페르소나 학습은 기본 켜짐(매일 04:30 KST, FxTwitter 읽기). `POST /x-ops/learn`으로 즉시 학습, `POST /x-ops/outbound`로 즉시 선댓글 틱. Telegram `/drill`은 `POST /api/internal/telegram/webhook`.
