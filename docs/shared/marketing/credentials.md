...
> **`bgm_track` (배경음악, 2026-08-23 추가)**: 같은 박스에서 고른다. 값은 WaggleBot 카탈로그의 `path`(`/api/media/bgm/{emotion}/{file}.mp3`)를 그대로 저장. 감정 5종(shock·anger·tension·sad·hype) × 2곡 = 10곡, Mixkit(상업 사용 가능·표기 불필요). **비워두면 사연의 `hook_emotion`에 맞춰 WaggleBot이 매번 자동 선택**하며, 값이 있을 때만 그 곡으로 고정(어드민 UI의 「자동 선택」 항목이 빈 값). **글로벌 on/off 스위치** `settings.yaml` 라인 23-25의 `bgm.enabled=false`로 전체 렌더에서 포함 여부 제어 (현재 false, 모든 렌더에서 제외). **음량 최적화** (ecb8722 커밋 2026-08-23): 원본 BGM이 사실상 들리지 않던 버그 수정 — `layout.py`의 volume 0.15→0.40, 더킹 threshold 0.03→0.10, ratio 9→4로 조정하여 최종 음량 -44.2 dB → -30.0 dB (목소리 -16.0 dB 대비 -14 dB 하향).
> **`instagram_reels` Graph API**: `app_id`/`app_secret`(Meta 앱) + `ig_user_id` + `access_token`(Instagram User/Page 토큰)은 **ASM credential AES-256-GCM만** SSOT. `.env`에 토큰·App Secret을 두지 않는다(YouTube `client_secret`/`refresh_token`과 동일). `graph_host` 기본=`graph.facebook.com`(Instagram Login이면 `graph.instagram.com`). Instagram Login은 공개 `video_url` 게시. 로컬 MP4는 Meta rupload(HTTP/1.1 · Content-Length · Expect 비활성). `ProcessingFailedError`는 **같은 컨테이너를 재사용하지 않고** 새 컨테이너로 최대 3회. Graph가 `User access is restricted`(25/2207050)를 주면 Playwright로 영상을 올리지 않고, `storage_state`로 `scraping_warning` **닫기**만 누른 뒤 Graph를 한 번 재시도한다.
