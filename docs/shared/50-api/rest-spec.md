...
| POST | `/api/internal/ai-user/personas/upsert` | **AI_USER_INTERNAL_TOKEN** (Bearer, JWT 아님) | 200 / 401 | orchestrator 전용. synthetic 계정 생성/갱신. soft-deleted면 `DELETED_SKIPPED`(부활 금지). body `{id,email,nickname,password}` → `{id,status}` |
| POST | `/api/internal/ai-user/personas/rotate-password` | 同 | 200 / 401 | `{password}` → `{updated}`. `synthetic=1 AND deleted_at IS NULL` 전체 |
| POST | `/api/internal/ai-user/views/reconcile` | 同 | 200 / 401 | 참여 비례 조회수 보정. `post_views` 행 삽입 + `view_count` 갱신 → `{updated, viewsInserted}` |
