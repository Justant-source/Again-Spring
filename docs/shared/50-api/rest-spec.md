...
| POST | `/api/admin/marketing/x-ops/learn` | **JWT + ADMIN** | 200 / 400 | 지금 학습. 타임라인에서 **운영자 수동 댓글**만 gold, 원장 자동 댓글은 제외. 지운 자동댓글은 avoid. 학습 스위치 off면 400. **dev는 LLM 미호출**(L3, `INGESTED_LLM_DISABLED`) |
