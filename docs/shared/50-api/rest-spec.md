...
| GET | `/api/admin/marketing/x-ops` | **JWT + ADMIN** | 200 | X 운영 설정. `{morningTime,nightTime,storyScoopsPerDay,outboundDailyCap,inboundDailyCap,inboundPerPostCap,hotMinReplies,hotMaxAgeHours,ritualEnabled,inboundEnabled,outboundEnabled,personaLearningEnabled,personaLearnAt,personaLastStatus,personaLastNewCount,personaLastLearnedAt,personaSummary,personaDrillToday}`. 키 `marketing.x.*`. 발행 스위치 기본 **false**. **선댓글 후보 조회는 주간 30분 간격**(08:00–22:00 KST), 의식/대댓글만 1분 틱. 페르소나 학습 기본 **true**·시각 `04:30` KST. `personaDrillToday`=오늘(KST) Telegram 드릴 저장 수. 원장 `x_ops_action` · 코퍼스 `x_persona_example`(어드민 REST 없음) |
| PUT | `/api/admin/marketing/x-ops` | **JWT + ADMIN** | 200 / 400 | 부분 갱신. 시각 `HH:mm`(KST). 기본: 아침 07:30 / 밤 22:00 / 사연 2 / 선댓글 20 / 대댓글 40·글당 12 / 최소댓글 3(0=제한없음)·6h / 학습 04:30 |
...
| POST | `/api/admin/marketing/x-ops/outbound` | **JWT + ADMIN** | 204 / 400 | 지금 선댓글 틱 1회(후보 조회+**최대 1건 게시**). 네이티브 영상(`hasVideo`)은 스킵 후 다음 후보. 드릴/코퍼스에 있는 tweetId는 스킵. 선댓글 스위치 off면 400. Playwright가 길 수 있음 |
| POST | `/api/internal/telegram/webhook` | **Telegram secret header** | 200 / 401 | Justant-Bot 드릴. `X-Telegram-Bot-Api-Secret-Token` 상수시간 비교. JWT 없음. `/drill`·`/skip`·드릴 메시지 답장. 즉시 200 후 비동기 처리 |
