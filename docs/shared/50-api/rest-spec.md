...
| POST | `/api/admin/marketing/x-ops/learn` | **JWT + ADMIN** | 200 / 400 | 지금 학습. 타임라인에서 **운영자 수동 댓글**만 gold, 원장 자동 댓글은 제외. 지운 자동댓글은 avoid. 증류 모델 **Sonnet**(`claude-sonnet-5`). 학습 스위치 off면 400. **dev는 LLM 미호출**(L3, `INGESTED_LLM_DISABLED`) |
| POST | `/api/admin/marketing/x-ops/outbound` | **JWT + ADMIN** | 204 / 400 | 지금 선댓글 틱 1회(후보 조회+**최대 1건 게시**). 네이티브 영상(`hasVideo`)은 스킵 후 다음 후보. 선댓글 스위치 off면 400. Playwright가 길 수 있음 |
