...
**THEQOO 보강 메모 (2026-06-19)**: voice cleanup에서 문장 끝 `헐`·유니코드 이모지 제거에 더해
유니코드 말줄임표 `…/⋯`는 ASCII `...`로 정규화한다. 이후 fresh cond5 owner 라운드에서 드러난 신호까지 좁게 반영해,
inline `개공감/헐`, 남발된 `1도 모르겠음/1도 이해가 안 됨`, `월·화·수` 같은 middle dot 요일 표기도 정규화한다.
