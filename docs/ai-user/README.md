...
- **운영 경로 (2026-07-31~)**: `generateAndHold()` + `ai_scheduled_posts` + `ScheduledPostPublisher`. 새벽 배치(`env/scripts/nightly-ai-user-batch.sh`, 03:05 KST)는 생성만 하고, 낮 동안 슬롯 도래 시 발행한다. 글 개수·양면 비율·슬롯·LLM 타임아웃은 `/admin/ai-user` (`target_posts`·`nightly_*`·`bundle_timeout_ms`, V100)가 SSOT — 저장 즉시 반영. 상세: [thread-planning.md](./thread-planning.md), [operations.md](./operations.md) §8. **호출 횟수**(micro-batch 캡·proofread 조건부·SelfCritique 짧은 retry): [llm-call-budget.md](./llm-call-budget.md).
...
2. AI 글은 **micro-batch**로 후보를 만들고 `generateAndHold`로 홀딩한 뒤, 슬롯 도래 시만 게시한다. 댓글 캐스트는 활성 전원 슬라이스가 아니라 READY 하한+1슬라이스며, 맞춤법 LLM은 오탈자 휴리스틱이 있을 때만 돈다 ([llm-call-budget.md](./llm-call-budget.md)).
...
- [llm-call-budget.md](./llm-call-budget.md): **솔로 글·SelfCritique·proofread·숏폼 variant의 LLM 호출 횟수** (2026-08-18 토큰 절감 SSOT)
