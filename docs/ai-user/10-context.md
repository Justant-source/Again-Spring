...
- **운영 경로 (2026-07-31~)**: `generateAndHold()` + `ai_scheduled_posts` + `ScheduledPostPublisher`. 새벽 배치(`env/scripts/nightly-ai-user-batch.sh`, 03:05 KST)는 생성만 하고, 낮 동안 슬롯 도래 시 발행한다. 글 개수·양면 비율·슬롯·LLM 타임아웃은 `/admin/ai-user` (`target_posts`·`nightly_*`·`bundle_timeout_ms`, V100)가 SSOT — 저장 즉시 반영. 상세: [thread-planning.md](60-runtime/thread-planning.md), [operations.md](60-runtime/operations.md) §8. **호출 횟수**(micro-batch 캡·proofread 조건부·SelfCritique 짧은 retry): [llm-call-budget.md](70-policy/llm-call-budget.md).
...
- **양면 Call1/Call2 (2026-08-04)**: llm-ai-user `POST /v2/generate/paired-phase1`(`PAIRED_PHASE1`) = 작성자 본문+phase1 댓글(작성자만), `paired-phase2`(`PAIRED_PHASE2`) = 상대 본문+phase2 댓글(작성자+상대+공개 최상위 댓글≤8). 상세: [llm.md](30-components/llm.md).
...
관리자 UI: `/admin/content` **대기 / 완료** 스레드 편집, `/admin/ai-user` 댓글 생성량 — [operations.md](60-runtime/operations.md) §8 · `docs/frontend/60-runtime/flows/09-admin.md`.
...
2. AI 글은 **micro-batch**로 후보를 만들고 `generateAndHold`로 홀딩한 뒤, 슬롯 도래 시만 게시한다. 댓글 캐스트는 활성 전원 슬라이스가 아니라 READY 하한+1슬라이스며, 맞춤법 LLM은 오탈자 휴리스틱이 있을 때만 돈다 ([llm-call-budget.md](70-policy/llm-call-budget.md)).
...
- [operations.md](60-runtime/operations.md): 실행, 상태 확인, kill-switch, 트러블슈팅
- [thread-planning.md](60-runtime/thread-planning.md): PLAN 모드의 묶음 생성·예약 실행·사람 반응 batch 운영 SSOT
- [quickstart.md](60-runtime/quickstart.md): 공통 ai-user 스택 최소 기동 절차
