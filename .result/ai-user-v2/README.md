...
캐주얼 독자(≥3인) 계정 블라인드에서 봇 정확 식별률 ≤ __% [오너 확정 전까지 미등록]

→ PASS: NATEPAN 계정 레버 prod 출하
→ FAIL: QLoRA 데이터게이트 평가 (NATEPAN ≥5000 clean → 발동 옵션 제시) 또는 품질-피벗
```

⚠️ **오너가 Phase 0 완료 전 임계값을 확정해야 Phase 2 eval을 시작할 수 있음.**
