...
**날짜**: 2026-06-22  
**상태**: ✅ **PASS 최종 확정 → SHIPPED** (naive 4인 평균 20% ≤ 60%). 오너 명시 출하 지시(2026-06-22)로 최종 확정.
...
- **20% ≤ 60% → PASS (잠정, margin 40pp).** 독립 재채점으로 동일 확인.
- 강건성: 신규 100% 만점자 5명 연속이어야 FAIL 전환. 평범한 ~50% 신규는 +8명도 안전.
- 평가자 판별력 사실상 상실: AI 식별 20% vs Human 오탐 55% (역전). Q3(실제 사람) 4/4 만장일치 AI 오판.
- 계정별: 035(MARRIED)·060(COUPLE) 완벽 위장 0/4. 057(FRIEND) 최다 적발 2/4(T3 잔존).
