...
**상태**: 2026-06-21 완료 (commit 4bc7c0cf)
...
> **연표 메모**: 이 섹션은 2026-06-21 기준 Phase 계획과 완료 기준을 보존한다. 현재 라이브 상태와 kill criterion 확정값은 `STATE.md`·`decisions.md`를 권위본으로 읽는다.
...
> **최종 (2026-06-22)**: 출하 시점 naive 4인 평균 AI 식별률 **20% ≤ 60% → PASS → SHIPPED**
> - 측정: 최일찬 40%·김태준 0%·김윤태 0%·윤도현 40% → 평균 20% (독립 재채점 일치)
> - 사후 누적: E-008 60%·E-009 80%·E-010 40%·E-011 80%·E-012 80% 추가 → 최신 9인 평균 **46.7% ≤ 60%**, PASS 유지
...
> - ⚠️ 인과 주의: Phase5→8 출하 시점은 −60pp, 최신 9인 기준은 −33.3pp지만 둘 다 5변수 동시변화 → "Phase 6 기여" 시사·미증명. 게이트 PASS는 절대 임계라 유효
...
**연표 메모**: 위 금지선은 kill criterion 확정 전 규율이다. 현재는 `V2-D05`와 `STATE.md` 기준으로 `≤60%` · `naive ≥3인` · `통합 평균`이 확정됐다.
