...
외부 사연(수동 수집) → 익명화(50%+ 재작성) → 7~10턴 Haiku 시뮬레이션 → V12 리포트 → 3채널 콘텐츠 자동 생성 → 채팅 UI 스크린샷 생성 → 3중 안전 검사 → 어드민 승인 → zip 다운로드 → 수동 발행
...
| 클래스 | 역할 |
|---|---|
| `ContentGenerator` (interface) | 플랫폼별 콘텐츠 생성 계약 |
...
| `XContentGenerator` | X(트위터) 3~5 트윗 스레드 생성 |
| `InstagramContentGenerator` | 인스타그램 캡션 + 해시태그 |
| `NaverBlogContentGenerator` | 네이버블로그 마크다운 |
| `PlatformContentRouter` | 진입점 (registry.resolve → generate) |
| `ContentGenerationExecutor` | 비동기 생성 + 스크린샷 |
...
| 채널 | 형식 | 제약 | 활성 |
|---|---|---|---|
| X (트위터) | 3~5 트윗 스레드 | 각 270자 이하, 해시태그 3개 | 활성 |
| 인스타그램 | 캡션 | 200~300자, 해시태그 5개 | 활성 |
| 네이버 블로그 | 마크다운 | 800~1,200자, 해시태그 5개 | 활성 |
| Threads | 단일 글 | 300자 이하, 해시태그 3개 | PR2 예정 |
| Facebook | 본문 | 400~600자, 해시태그 2개 | PR2 예정 |
...
| `POST /render-chat` | `{messages, title, subtitle, viewport}` | PNG bytes | 다시봄 채팅 UI 스크린샷 |
