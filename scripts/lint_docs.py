...
BANNED_DOC_REFS: list[tuple[re.Pattern, str]] = [
    # 기존 check-docs.js 패턴: keywordGuard 컴포넌트 금지 참조
    (re.compile(r"->\s*`KeywordGuard`\s*컴포넌트|`frontend/lib/utils/keywordGuard\.ts`|입력\s*필드.*keywordGuard|keywordGuard.*체크리스트", re.I),
     "KeywordGuard 컴포넌트 — lib/utils/keywordGuard.ts 는 2026-06-14 삭제됨"),
]
