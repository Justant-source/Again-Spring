...
interface LandingPageClientProps {
  /** 서버(page.tsx)에서 fetch한 "방금 올라온 사연" 초기값 — 크롤러가 첫 HTML에서 바로 본다 */
  initialLatestPost: PostSummary | null;
  /** 서버에서 fetch한 "오늘의 사연" 초기값 */
  initialTodayPost: PostSummary | null;
}

export default function LandingPageClient({ initialLatestPost, initialTodayPost }: LandingPageClientProps) {
  const router = useRouter();
  const user = useUserStore((s) => s.user);
  const [mounted, setMounted] = useState(false);
  const [latestPost, setLatestPost] = useState<PostSummary | null>(initialLatestPost);
  const [todayPost, setTodayPost]   = useState<PostSummary | null>(initialTodayPost);
  useGuestInit();

  useEffect(() => {
    setMounted(true);

    // 방금 올라온 사연 = 최신순 1건 (서버 초기값을 클라이언트에서 최신 상태로 갱신)
  ...
  // 로그인 사용자별 UI(관리자 진입 버튼)는 hydration 이후에만 계산한다 —
  // 서버는 어떤 사용자인지 모르므로 여기서 미리 결정하면 사용자 간 캐시가 섞인다.
  const perms = permissionsFor(mounted ? user : null);
  const showAdminEntry = mounted && perms.ui.showAdminEntryButton;
  ...
  return (
    <div style={{ background: 'var(--L-bg)', minHeight: '100dvh', display: 'flex', flexDirection: 'column' }}>
      <div className="flex flex-col flex-1 px-7 pt-6 pb-5" style={{ maxWidth: 640, margin: '0 auto', width: '100%' }}>
        {/* 헤더: 다시봄 + 우측 유저 칩 — user는 mounted 이후에만 반영(하이드레이션 불일치 방지) */}
        <BrandBar title="다시봄" user={mounted ? user : null} />
