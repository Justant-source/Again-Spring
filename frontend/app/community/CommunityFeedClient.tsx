...
interface CommunityFeedClientProps {
  /** 서버(page.tsx)가 sortBy=latest, category 없음으로 fetch한 1페이지 — 크롤러가 첫 HTML에서 바로 본다 */
  initialPosts: PostSummary[];
  /** 서버 fetch 결과 totalPages > 1 여부 — 초기 hasMore 값 */
  initialHasMore: boolean;
}

export default function CommunityFeedClient({ initialPosts, initialHasMore }: CommunityFeedClientProps) {
  ...
  // 서버(page.tsx)가 이미 기본 필터(전체·최신순)로 1페이지를 가져왔다 — 마운트 직후
  // 같은 조건으로 또 한 번 fetch하는 낭비를 막는다. 필터가 바뀌는 순간부터는 정상 동작.
  const skipInitialFetchRef = useRef(initialPosts.length > 0);

  const categoryOptions = [{ id: '', label: '전체' }, ...C3_CATS];

  useEffect(() => {
    if (skipInitialFetchRef.current && selectedCategory === '' && sort === 'latest') {
